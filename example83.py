...
def manual_str_formatting(name, subscribers):
    if subscribers > 100000:
        ...
        print("lol: " + name + " thats not many subs!")

    #better
    if subscribers > 100000:
        print(f"Wow {name}! you have {subscribers} subscribers! ")
    else:
        print(f"lol {name} thats not many subs!")



def manaully_calling_close_on_a_file(filename):
    f = open(filename, "w")
    f.write("hello:\n")
    f.close()
...
def always_using_comprehensions(a, b, n):
    c = [
        sum(a[n * i * j * k] * b[n * k + j] for k in range(n))
        for i in range(n)
        for j in range(n)
    ]
