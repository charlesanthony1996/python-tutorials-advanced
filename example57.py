...
def name_binding_scope():
    ...
    print(f"{x} is undefined/implementation dependent behaviour!")
...
def timing_match_by_class_old():
    ...
    elif isinstance(event, Click) and event.key_name in ["Q", "q"]:
        x, y = event.position

    elif isinstance(event, KeyPress) and event.key_name == "up arrow" or isinstance(event, Quit):
        pass
