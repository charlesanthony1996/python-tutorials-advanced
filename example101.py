...
@overload
def dict_zip(m1: Mapping[_K, _V1],) -> Iterator[tuple[_K, _V1]]:
    ...

@overload
def dict_zip(m1: Mapping[_K, _V1],) -> Iterator[tuple[_K, _V1, _V2]]:
    ...



@overload
def dict_zip(m1: Mapping[_K, _V1], m2: Mapping[_K, _V1, _V2], m3 : Mapping[_K, _V1, _V2]) -> Iterator[tuple[_K, _V1, _V2, _V3]]:
    ...
...
def dict_zip_union():
    ...
    for key in keys:
        yield key, *(d.get(key, filename) for d in dicts)
...
def seperate_dicts_example_1():
    ...
    for cid in names:
        ...
        print(f"{name} has {sub_count} subcribers! Watch here : youtube.com/channel/{cid}")
