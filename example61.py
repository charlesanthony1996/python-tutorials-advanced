...
def crack_length(user, max_len=32, verbose=False) -> int:
    ...
    for i in range(max_len):
        i_time = timeit.repeat(stmt="check_password(user, x)", setup=f"user={user!r};x=random_str({i!r})", globals=globals(), number=trials, repeat=10)
...
def crack_password(user, length, vebose=False):
    ...
    while True:
        ...
        for c in allowed_chars:
            ...
            alt_time = timeit.repeat(stmt="check_password(user, x)", setup=f"user={user!r};x={alt!r}", globals =globals(), number = trials, repeat=10)

            guess_time = timeit.repeat(stmt="check_password(user, x)", setup=f"user={user!r};x={guess!r}", globals= globals(), number = trials, repeat=10)
