...
def etl(filename:str) -> tuple[str, float]:
    ...
    samplerate , data = scipy.io.wavfileread(filename)
...
def run_with_mp_map(items, do_work , processes= None, chunksize=None):
    print(f"running using mutliprocessing with {processes=}, {chunksize=}")
    start_t = time.perf_counter()
    with Pool(processing = processes) as pool:
